    ...
    const myFleetCards=_orderedFleet.map(({s,origIdx:i})=>{
      ...
      return '<div style="background:'+cardBg+';border:1px solid '+cardBdr+';border-radius:10px;padding:10px;display:flex;flex-direction:row;gap:8px;align-items:stretch">'+
        ...
        '<div style="flex:1;display:flex;flex-direction:column;gap:5px;min-width:0">'+
          ...
          '</div>'+
        '</div>'+
        // ── 오른쪽: 이미지 ── (사용자 요청 2026-06-08: 정비소 모드 추가 1.5배 확대 330→495)
...
function renderShipSkinTab(body){
  ...
  const fleetList=G.fleet.map((s,i)=>{
    ...
    return `<div onclick="_selectedSkinShipIdx=${i};rerenderTab(renderGarageTab)" style="background:${sel2?'rgba(0,243,255,.14)':'var(--card)'};border:1.5px solid ${sel2?'var(--cyan)':fc+'55'};border-radius:8px;padding:8px;cursor:pointer;display:flex;gap:12px;align-items:center;margin-bottom:6px">
      <div style="width:216px;height:216px;border-radius:12px;overflow:hidden;flex-shrink:0">${imgOrEmoji(shipImgSrc(s),'🛸',216,216,'object-fit:cover;width:100%;height:100%')}</div>
      <div style="flex:1;min-width:0">
  ...
  // 스킨 제거(원본 복귀) 옵션
  const removeBtn=(sel&&sel._skinCatId)?`<div style="margin-bottom:10px"><button class="btn btn-sm" style="border-color:var(--red);color:var(--red);font-size:12px;padding:5px 14px" onclick="removeShipSkin(${_selectedSkinShipIdx})">${I18N.t('ui.removeSkinBtn')}</button></div>`:'';
  body.innerHTML=`<div class="hub-scroll">
    ${hubBanner('garage','🔧',I18N.t('ui.shipMaintenance'),pd?.f)}
  ...
  if(!sel){
    ...
    if(isMax){
      ...
      rightPanel=`<div style="background:rgba(0,0,0,.3);border:1px solid var(--bdr);border-radius:10px;padding:14px">
        ...
        <button class="btn btn-gold" style="width:100%;font-size:14px;padding:10px" onclick="doShipEnhance(${_selectedEnhanceShipIdx})" ${canEnhance?'':'disabled'}>${canEnhance?I18N.t('enhance.tryLv',{lv:nextLv}):(creditsOk?I18N.t('enhance.lackMaterial'):I18N.t('enhance.lackCredits'))}</button>
      </div>`;
    }
  }
  body.innerHTML=`<div class="hub-scroll">
...
function renderCombatLog(body){
  ...
  body.innerHTML=`<div class="hub-scroll">${hubBanner('clog','📋',I18N.t('hub.bannerCombatRecord'))}<div class="hub-t">${I18N.t('hub.combatLog')}</div>
