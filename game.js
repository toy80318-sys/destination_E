...
function renderMain(body){
  ...
  try{body.innerHTML=`
    <div style="display:flex;gap:8px;padding:8px 14px;background:rgba(13,26,42,.98);border-bottom:1px solid var(--bdr);flex-wrap:nowrap;overflow-x:auto;flex-shrink:0;scrollbar-width:none">
      ...
      ${(()=>{const rep=G.reputation||0,rr=getRepRank(rep);const next=rr.next;const pct=Math.min(100,next>0?Math.round(rep/next*100):100);const _ver=window._GAME_VER?'?v='+window._GAME_VER:'';return`<div class="ic" style="min-width:130px;display:flex;align-items:center;gap:8px"><img src="img/ui/HN01.png${_ver}" alt="HN" style="width:32px;height:32px;object-fit:contain;flex-shrink:0;filter:drop-shadow(0 0 6px ${rr.col})" onerror="this.style.display='none'"><div style="flex:1;min-width:0"><span class="icl">${I18N.t('ui.repShort')}</span><span class="icv" style="color:${rr.col};font-weight:bold">${rr.ic} ${rr.lb}</span><div style="margin-top:2px;height:3px;background:rgba(255,255,255,.1);border-radius:2px;overflow:hidden"><div style="height:100%;width:${pct}%;background:${rr.col};transition:width .4s"></div></div><div class="ic-sub" style="font-size:9px;color:var(--dim);text-align:right;margin-top:1px">${rep}${next<9999?' / '+next:''}</div></div></div>`;})()}
