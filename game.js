...
// ═══ PRNG & MAP ══════════════════════════════════════════════════
// ─── mulberry32 + generateGalaxy + buildConnections → js/modules/galaxy-gen.js 로 분할 (2026-06-17) ───
//   세 함수 모두 전역(window)으로 노출 — game.js·starmap.js 보다 먼저 로드된다.
...
// ═══ TITLE / AGE / FTUE — js/modules/title-flow.js 로 분리 (C7, 2026-06-23, 긴 코드 분할) ═══
//   goAgeGate/showExitModal/showTitle/exitFromTitle/checkAge/setGender/setDifficulty/
//   changeDifficultyFromSettings + DIFF_LIST/DIFF_COLORS → title-flow.js 로 이동.
//   전역(window) 함수라 onclick 호출부 변경 불필요. index.html 에서 game.js 직후 로드.
...
function renderMain(body){
  ...
  try{body.innerHTML=`
    <div class="hub-chip-row" style="display:flex;gap:8px;padding:8px 14px;background:rgba(13,26,42,.98);border-bottom:1px solid var(--bdr);flex-wrap:nowrap;overflow-x:auto;flex-shrink:0;scrollbar-width:none">
      ...
      ${(()=>{const lv=calcPlayerLevel(),rk=getLevelRank(lv);return`<div class="ic"><span class="icl">${I18N.t('ui.levelShort')}</span><span class="icv" style="color:${rk.col};font-weight:bold">Lv.${lv} <span class="ic-sub" style="font-size:12px">${rk.lb}</span></span></div>`;})()}
      ${(()=>{const rep=G.reputation||0,rr=getRepRank(rep);const next=rr.next;const pct=Math.min(100,next>0?Math.round(rep/next*100):100);const _ver=window._GAME_VER?'?v='+window._GAME_VER:'';return`<div class="ic ic-rep" style="min-width:130px;display:flex;align-items:center;gap:8px"><img src="img/ui/HN01.png${_ver}" alt="HN" style="width:32px;height:32px;object-fit:contain;flex-shrink:0;filter:drop-shadow(0 0 6px ${rr.col})" onerror="this.style.display='none'"><div style="flex:1;min-width:0"><span class="icl">${I18N.t('ui.repShort')}</span><span class="icv" style="color:${rr.col};font-weight:bold">${rr.ic} ${rr.lb}</span><div style="margin-top:2px;height:3px;background:rgba(255,255,255,.1);border-radius:2px;overflow:hidden"><div style="height:100%;width:${pct}%;background:${rr.col};transition:width .4s"></div></div><div class="ic-sub" style="font-size:9px;color:var(--dim);text-align:right;margin-top:1px">${rep}${next<9999?' / '+next:''}</div></div></div>`;})()}
    ...
    <div style="background:rgba(13,26,42,.97);border-top:1px solid var(--bdr);flex-shrink:0">
      <div class="hub-status-strip" style="display:flex;align-items:center;gap:14px;padding:4px 16px;overflow-x:auto;height:36px;border-bottom:1px solid rgba(0,243,255,.1)">
...
// ═══ ECONOMY — js/modules/economy.js 로 분리 (C4, 2026-06-23, 긴 코드 분할) ═══
//   투자비·세금, 해적/치크스 조우·습격, 화물/재료 정합성, 매각가 등 전역 함수 25개를
//   js/modules/economy.js 로 이동. 전역(window) 함수라 호출부 변경 불필요.
//   index.html 에서 game.js 직후(combat.js 인근) 로드. economy.js 는 런타임에
//   game.js 잔류 전역(G, PLANET_DEF, combatState, getShipStats, RARITY_MULT,
//   CREW_BONUS_TABLE, clamp, updateHUD, notify, I18N 등)을 참조한다.
...
// ═══ VIEWS-MISC — js/modules/views-misc.js 로 분리 (C5, 2026-06-23, 긴 코드 분할) ═══
//   제작 품질/폭죽(rollCraftQuality, _shipCraftSuperiorBonus, _fireFireworks),
//   잠긴 메뉴 NPC 대사(_getLockedNpcDialog), 행성 광장/전방 뷰(renderPlazaView, renderFrontView),
//   전투 기록 뷰(renderCombatLog, _combatLogContentHTML) → views-misc.js 로 이동.
//   전역(window) 함수라 호출부 변경 불필요. index.html 에서 game.js 직후 로드.
// ═══ HERO RECRUIT — js/modules/hero-recruit.js 로 분리 (C6, 2026-06-23, 긴 코드 분할) ═══
//   HERO_GREETING + showHeroRecruit/boardHeroToShip/unassignHero/_grantAllHeroesReward/
//   recruitHero/applyJangYeongsilEffect → hero-recruit.js 로 이동.
//   전역(window) 함수라 호출부 변경 불필요. index.html 에서 game.js 직후 로드.
