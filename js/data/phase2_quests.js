...
const PHASE2_QUESTS={
  ...
  P19:[
    ...
    // ── Q06-02 잔해 구역 탐색 (서브) ──
    {
      ...
      desc:{ko:'마르코: "전투 잔해 구역에 치크스 화물이 남아있어. 파편을 모아두면 나중에 써먹을 수 있어."\nG17 ×1 소비(탐색 시동) → 잔해 탐색 → 치크스 결정 파편 ×2 + 치크스 결정석 ×1 회수.',
    ...
    // ── Q06-04 포자 압박 해결 (메인) ──
    {
      ...
      desc:{ko:'마르코: "치크스가 내 화물칸에 포자를 강제 하역하라고 압박 중이야. 지휘선에서 잠금 해제 코드를 빼와."\n치크스 지휘선 강습 → 코드 탈취 + 뇌수액 ×2 부수 노획.',
            en:'Marco: "Cygnus forces Chiks Mutation Spore spores onto my hold. Steal the override code." Storm command ship.'},
      objectives:[
    ...
    // ── Q06-M 마르코의 칩 해독 (영웅 신뢰 — 이미 합류 상태 · 아이템 제작) ──
    {
      ...
      desc:{ko:'페이즈 1 바나드 프라임에서 합류한 마르코, 우르사 작전 완수 후 신뢰 한 단계 깊어짐.\n마르코 작업대에서 아우레우스 정보 칩 해독 — 치크스 결정 파편 ×2 소비(암호 해석 매체 제작) → 칩 해독 성공.\n해독 결과: "크리그 스파이가 로스 128-b 수퍼비아 집하장 —32구역 경유."',
            en:'Marco (already joined Phase 1) deepens trust. Chiks Crystal Shard ×2 → decode Aureus Information Chip chip → Krieg spy route to Ross 128-b zone-32.'},
      objectives:[
  ...
  P04:[
    ...
    // ── Q07-03 중력 시스템 응급 복구 (서브) ──
    {
      ...
      desc:{ko:'오스카르: " 중력수정이 오정렬됐어. 집하장 컨베이어 전체가 멈출 것 같아."\nG05 ×2 제공 → 시스템 복구.',
  ...
  P13:[
    ...
    // ── Q10-02 지하 감옥 진입 (메인 전투) ──
    {
      ...
      desc:{ko:'크리그 전투 자극제 ×1 소비(크리그 위장 약물) → 감옥 층 접근.\nG11 ×1 소비(경비 반장 매수 시도) → 거절 → 전투 → 격퇴.\n지하 아카이브에서 거북선 설계도 조각 5% 드롭.',
            en:'Krieg Combat Stimulant ×1 disguise + Krieg Bloodstone ×1 bribe attempt → refused → fight. Geobukseon fragment 5% drop in archive.'},
      objectives:[
    ...
    // ── Q10-03 이순신 캡슐 봉인 해제 (메인 특수 · G18 필요) ──
    {
      ...
      desc:{ko:'이순신 캡슐 인식 코드: 난중일기 영인본 필요.\nG18 소비(캡슐 인식 장치 활성화) → 봉인 해제.\n이순신: "이곳이... 얼마나 흘렀느냐. 나를 깨운 자가 {사령관}인가. 빚을 졌구나."',
            en:'Capsule code: "Nanjung Ilgi" Facsimile Nanjung Diary. "Nanjung Ilgi" Facsimile ×1 → unseal. Yi Sun-sin: "How long has it been... I owe you a debt."'},
      objectives:[
        {type:'gather', item:'G18', qty:1, label:{ko:'난중일기 영인본 ×1', en:'"Nanjung Ilgi" Facsimile Nanjung Diary ×1'}},
      ],
      rewardCr:30000, rewardVe:80,
      rewardItems:[],
      ...
      lockReason:{ko:'페이즈 3 저항군 기지(저항군 지하 방공호)에서 난중일기 영인본을 먼저 입수하라.',
                  en:'Acquire "Nanjung Ilgi" Facsimile Nanjung Diary at Resistance base (Resistance Underground Bunker) in Phase 3 first.'},
      cutscene_pre:null, cutscene_post:null
    },
    // ── Q10-M 이순신(H01) 합류 (영웅 퀘스트 + 세트 보상) ──
    {
      ...
      heroId:'H01',
      ic:'⚔️', img:'img/quests/yi_join.png', npc:'이순신', npcIc:'🛡️', npcKey:'hero01',
...
const PHASE2_CUTSCENES_KO={
  ...
  p2_ch04c:[
    {char:'hero07', name:'테슬라', color:'#66ffff', text:'크리그가 이순신 장군 캡슐을 왜 봉인했는지 알아? 이순신 장군은 수백 년 전 — 치크스와 우르사 메이저가 지구를 봉쇄하고, 그 하수인인 크리그 아이젠클로가 외곽 항로를 틀어막을 때 — 끝까지 저항했던 유일한 존재야. 그 전술 데이터를 아이젠클로가 강제 추출해 크리그 방어 알고리즘에 통합했어. 살아있는 방패인 거야. 이순신 장군이 살아서 움직이면 크리그 방어 알고리즘이 스스로 무너질 수도 있어.'},  // 수정 2026-06-13: 크리그=치크스 하수인 군벌로 격하 (메인 빌런 프레이밍 제거) — GDD: 치크스/우르사 메이저가 메인
