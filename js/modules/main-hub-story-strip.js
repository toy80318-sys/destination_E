...
function _buildStoryQuestStripHTML(pid){
  try{
    ...
    const _cards=_stories.map(function(q){
      ...
      // sq-* 클래스: 모바일 타이포 하한 보정 훅 (game.css body.is-mobile .sq-*) — 인라인 값은 데스크톱 기본값 유지
      const _prog=q.status==='active'?('<div class="sq-prog" style="font-size:10px;color:#cc88ff;margin-top:2px">'+_objLbl+' · '+(q.progress||0)+'/'+(q.required||1)+'</div>'):'';
      return '<div class="sq-card" onclick="_openStoryQuestDetail(\''+pid+'\',\''+_qid+'\')" style="flex:0 0 280px;background:linear-gradient(135deg,rgba(170,80,255,.22),rgba(40,10,80,.92));border:1.5px solid '+_col+';border-radius:8px;padding:8px 10px;cursor:pointer;'+_glow+'transition:transform .15s" onmouseover="this.style.transform=\'translateY(-2px)\'" onmouseout="this.style.transform=\'translateY(0)\'" title="'+_nm+' — '+(_en?'click for details':'클릭하여 상세 보기')+'">'
        +'<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span class="sq-ic" style="font-size:16px">'+(q.ic||'⭐')+'</span><span class="sq-title" style="font-weight:bold;color:#e0b3ff;font-size:13px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+_nm+'</span><span class="sq-badge" style="font-size:9px;padding:1px 5px;border:1px solid '+_col+';color:'+_col+';border-radius:4px;flex-shrink:0">'+_catLbl(_cat)+'</span></div>'
        +'<div class="sq-desc" style="font-size:11px;color:#bbb;line-height:1.35;max-height:30px;overflow:hidden">'+_desc+'</div>'
        +_prog
        +'<div style="display:flex;justify-content:space-between;align-items:center;margin-top:6px"><span class="sq-state" style="font-size:11px;color:'+_col+';font-weight:bold">'+_stLbl(q.status)+'</span><span class="sq-reward" style="font-size:11px;color:var(--gold)">+₡'+(q.rewardCr||0).toLocaleString()+' · VE+'+(q.rewardVe||0)+'</span></div>'
        +'</div>';
    }).join('');
    return '<div class="sq-strip" style="position:absolute;bottom:12px;left:12px;right:12px;z-index:15;display:flex;gap:8px;overflow-x:auto;padding:6px 4px;background:linear-gradient(180deg,transparent,rgba(13,26,42,.88) 30%);scrollbar-width:thin;scrollbar-color:#cc88ff44 transparent" data-scroll-id="story-strip">'
      +'<div class="sq-strip-label" style="position:sticky;left:0;display:flex;align-items:center;gap:4px;padding:6px 10px;background:rgba(80,40,140,.7);border:1px solid #cc88ff;border-radius:8px;color:#e0b3ff;font-size:11px;font-weight:bold;flex-shrink:0;white-space:nowrap;z-index:2">📜 '+(_en?'Scenario':'시나리오')+' '+_stories.length+'</div>'
      +_cards
...
function _openStoryQuestDetail(pid,qid){
  try{
    ...
    const _bodyHtml='<div style="padding:4px 2px"><div style="font-size:11px;color:#cc88ff;font-weight:bold;margin-bottom:4px">'+_catLbl+' · NPC: '+(_q.npcIc||'')+' '+(window._npcName?window._npcName(_q.npc||''):_sub(_q.npc||''))+'</div><div style="font-size:13px;color:var(--txt);line-height:1.6;margin-bottom:8px;white-space:pre-line">'+_sub(_q.desc||'')+'</div><div style="font-size:11px;color:var(--dim);margin-top:8px">'+(_en?'Objectives:':'목표:')+'</div>'+_objHtml+_rewardHtml+'</div>';
    const _btns=[];
