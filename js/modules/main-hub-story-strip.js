...
function _buildStoryQuestStripHTML(pid){
  try{
    ...
    function _catLbl(cat){return cat==='main'?'[메인]':cat==='hidden'?'[히든]':'[서브]';}
    function _stLbl(st){return st==='available'?'🆕 수락 가능':st==='done'?'✅ 보상 수령':'⏳ 진행 중';}
    const _cards=_stories.map(function(q){
      ...
      const _desc=(q.desc||'').toString().substring(0,80).replace(/"/g,'&quot;');
      const _obj0=(q.objectives&&q.objectives[0])||{};
      const _objLbl=(typeof _obj0.label==='object'?(_obj0.label.ko||_obj0.label.en||''):(_obj0.label||'')).toString();
      const _prog=q.status==='active'?('<div style="font-size:10px;color:#cc88ff;margin-top:2px">'+_objLbl+' · '+(q.progress||0)+'/'+(q.required||1)+'</div>'):'';
      return '<div onclick="_openStoryQuestDetail(\''+pid+'\',\''+_qid+'\')" style="flex:0 0 280px;background:linear-gradient(135deg,rgba(170,80,255,.22),rgba(40,10,80,.92));border:1.5px solid '+_col+';border-radius:8px;padding:8px 10px;cursor:pointer;'+_glow+'transition:transform .15s" onmouseover="this.style.transform=\'translateY(-2px)\'" onmouseout="this.style.transform=\'translateY(0)\'" title="'+_nm+' — 클릭하여 상세 보기">'
    ...
    return '<div style="position:absolute;bottom:12px;left:12px;right:12px;z-index:15;display:flex;gap:8px;overflow-x:auto;padding:6px 4px;background:linear-gradient(180deg,transparent,rgba(13,26,42,.88) 30%);scrollbar-width:thin;scrollbar-color:#cc88ff44 transparent" data-scroll-id="story-strip">'
      +'<div style="position:sticky;left:0;display:flex;align-items:center;gap:4px;padding:6px 10px;background:rgba(80,40,140,.7);border:1px solid #cc88ff;border-radius:8px;color:#e0b3ff;font-size:11px;font-weight:bold;flex-shrink:0;white-space:nowrap;z-index:2">📜 시나리오 '+_stories.length+'</div>'
      +_cards
...
function _openStoryQuestDetail(pid,qid){
  try{
    ...
    if(!_q){if(typeof window.notify==='function')window.notify('시나리오 퀘를 찾을 수 없습니다: '+qid,'err');return;}
    const _idx=_qlist.indexOf(_q);
    const _cat=_q.category||'sub';
    const _catLbl=_cat==='main'?'[메인 시나리오]':_cat==='hidden'?'[히든 시나리오]':'[서브 시나리오]';
    const _objHtml=_q.objectives&&_q.objectives.length?_q.objectives.map(function(o){
      const _lbl=(typeof o.label==='object'?(o.label.ko||o.label.en||''):(o.label||'')).toString();
    ...
    const _rewardHtml='<div style="margin-top:10px;padding:8px 10px;background:rgba(212,175,55,.08);border:1px solid rgba(212,175,55,.3);border-radius:6px"><div style="font-size:11px;color:var(--dim);margin-bottom:4px">보상</div><div style="display:flex;gap:10px;flex-wrap:wrap;font-size:13px"><span style="color:var(--gold);font-weight:bold">+₡'+(_q.rewardCr||0).toLocaleString()+'</span><span style="color:var(--cyan);font-weight:bold">VE+'+(_q.rewardVe||0)+'</span>'+((_q.rewardItems||[]).map(function(it){return '<span style="color:#88ff88">📦 '+(it.id||'')+' ×'+(it.qty||1)+'</span>';}).join(''))+'</div></div>';
    const _bodyHtml='<div style="padding:4px 2px"><div style="font-size:11px;color:#cc88ff;font-weight:bold;margin-bottom:4px">'+_catLbl+' · NPC: '+(_q.npcIc||'')+' '+(_q.npc||'')+'</div><div style="font-size:13px;color:var(--txt);line-height:1.6;margin-bottom:8px;white-space:pre-line">'+(_q.desc||'')+'</div><div style="font-size:11px;color:var(--dim);margin-top:8px">목표:</div>'+_objHtml+_rewardHtml+'</div>';
    const _btns=[];
    if(_q.status==='available'){
      _btns.push({txt:'✓ 수락',cls:'btn-green',fn:function(){
      ...
      _btns.push({txt:'💰 보상 수령',cls:'btn-gold',fn:function(){
        if(typeof window.closeModal==='function')window.closeModal();
        if(typeof window.completeQuest==='function')window.completeQuest(pid,_idx);
        try{if(G._currentHubTab==='main' && typeof window.rerenderTab==='function' && typeof window.renderMain==='function')window.rerenderTab(window.renderMain);}catch(e){}
      }});
    }
    _btns.push({txt:'퀘 탭 열기',cls:'',fn:function(){if(typeof window.closeModal==='function')window.closeModal();if(typeof window.hubTab==='function')window.hubTab('quest');}});
    _btns.push({txt:'닫기',cls:'',fn:function(){if(typeof window.closeModal==='function')window.closeModal();}});
    if(typeof window.openModal==='function'){
      window.openModal((_q.ic||'⭐')+' '+(_q.nm||'시나리오 퀘'),_bodyHtml,_btns);
    }
