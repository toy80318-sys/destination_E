...
## 12. 진행 현황 — 1단계 모바일 UI 적응 (2026-06-28)

**실기 모바일 가로(844×390) 렌더 감사:** 타이틀·컷신·허브(사이드바+HUD+퀘스트카드+백구바) 모두 **스테이지 contain-fit(중앙·레터박스)으로 정상 표시**. 기존 `@media (max-width:768px),(pointer:coarse)` 블록이 버튼·사이드바·HUD·모달·칩·백구바를 이미 터치 친화적으로 스케일 중 → 큰 재작업 불필요 확인.

**이번에 반영(코드):**
- **세이프에어리어(노치/홈바) 대응** — `game.js` `_safeInsets()` 추가, `fitGameStage()`가 `env(safe-area-inset-*)`를 읽어 가용 영역에서 대칭 차감 후 스케일 산출. 데스크톱·비노치 기기는 인셋 0 → 동작 불변(회귀 검증: 844×390 스케일 0.52 유지).
- **타이틀 메뉴 터치 크기** — `game.css` `@media (pointer:coarse)`로 `.tmenu-main/.tmenu-wide/.tmenu-box` 글씨 확대.

**다음 단계(미진행 — 환경/결정 필요):**
- **Capacitor 스캐폴드 + Android 프로젝트**(이 Windows에서 가능) / **iOS 프로젝트**(Xcode·Mac 필수 — 생성은 가능하나 빌드·제출은 Mac).
- 정보 밀집 화면(정비소·도감·경매) 소형화면 재배치(§11), 스타맵 핀치줌, 전투 HUD 엄지영역 배치 — 실기 피드백 기반 폴리시.
